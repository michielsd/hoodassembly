...
add_line = """INSERT INTO mboschool (Brinnummer,  Instellingsnaam ,  Straatnaam,  Huisnummer,  Plaatsnaam,  Gemeentenaam,  Denominatie,  Telefoonnummer,  Internetadres) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)"""

with open(file, 'r', newline='') as csvfile:
    inserttable = list(list(rec) for rec in csv.reader(csvfile, delimiter=';', quotechar='"'))

    for row in inserttable[1:]:
        selectrow = row[4:8] + [row[9].capitalize()] + [row[11].capitalize()] + row[12:15]
