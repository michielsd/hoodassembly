...
add_line = """INSERT INTO hboschool (Brinnummer,  Instellingsnaam ,  Straatnaam,  Huisnummer,  Plaatsnaam,  Gemeentenaam,  Denominatie,  Telefoonnummer,  Internetadres) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)"""

with open(file, 'r', newline='') as csvfile:
    inserttable = list(list(rec) for rec in csv.reader(csvfile, delimiter=';', quotechar='"'))

    for row in inserttable[1:]:
        selectrow = row[3:7] + [row[8].capitalize()] + [row[10].capitalize()] + row[11:14]
